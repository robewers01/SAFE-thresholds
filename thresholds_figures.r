...
png('figures/figS2.png', width = 800, height = 800)
{
	...
	#Panel D - taxa among datasets
	{
		...
		predx <- -200:300
		x_pp <- D(x_p, 'predx')
		mod.d1 <- with(mod, eval(x_p))
		mod.d2 <- with(mod, eval(x_pp))
		fits <- data.frame (agb = predx, obs = rep(1, length(predx)), d1 = mod.d1, d2 = mod.d2)
		fitmod <- list(fits = fits)
		turns <- root.finder(fitted_vals = fitmod)
		predx <- turns$tps
		mod.d2 <- with(mod, eval(mod_expr2))
		predx <- turns$maxrate
		mod.d1 <- with(mod, eval(mod_expr2))
		points(turns$tps, mod.d2, pch = 17, lwd = 2, cex = 5, col = alpha(pal[3], 0.8))
		points(turns$maxrate, mod.d1, pch = 25, lwd = 2, cex = 5, col = alpha(pal[3], 0.8))
