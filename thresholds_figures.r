...
png('figures/fig2.png', width = 1600, height = 800, res = 150)
{
	...
	#Panel A: Taxonomic resilience
	{
		...
		points(mean.turn, prop_imp, cex = 5*tax_res^2, pch = 19, , col = alpha(1, 0.6))	
		...
		legend('bottomright', pch = 19, pt.cex = 5*c(0.45, 0.6, 0.75, 0.9)^2, legend = c('0.45', '0.60', '0.75', '0.90'),
	...
	resil_func$funcs$col <- as.numeric(resil_func$funcs$category)
	...
	points(resil_func$sens$mean.turn, resil_func$susc$prop_imp, 
		cex = resil_func$funcs$resilience^2 * 5, pch = 19, col = alpha(pal[resil_func$funcs$col], 0.7)) 
	
	legend('bottomright', pch = 19, legend = unique(resil_func$funcs$category),
		col = alpha(pal[unique(resil_func$funcs$col)], 0.7),
...
##FIGURE S1 - PHYLOGENY
png(paste('figures/figS1.png', sep = ""), width = 1000, height = 1000, res = 150)
{
