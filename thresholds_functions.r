...
fit.glmer <- function(comm, predictor){
	...
	for(scale in 1:length(predictor)){						#For each predictor...
		assign('pred', comm[, which(names(comm) == predictor[scale])])
		model <- try(glmer(comm[,responseInd] ~ pred + (1 + pred|comm$survey), family = binomial), FALSE)
		assign(paste('mod', scale, sep = ""), model)
		aic <- c(aic, AIC(model))
		rm(model)
		}
		if(class(model) != "try-error"){
			...
			rm(model)
			  ...
			  aic <- c(aic, NA)
		  mod0 <- try(glmer(comm[,responseInd] ~ 1 + (1 + 1|comm$survey), family = binomial), FALSE)	#Fit null model
	#Select best model
	if(sum(diff(aic)) == 0){		#Where all models give same AIC or there's only a single model
		best <- mod1		#Select model with smallest spatial scale
		bestpred <- predictor[1]
...
glmer.extract <- function(glmer_output, taxa_data, comm = comm, taxon, coefs = NA){
	...
	taxon <- taxon
	modtype <- 'glmer'
	r2 <- 	r.squaredGLMM(glmer_output$bestmod)[1,1]
	num.obs <- nrow(comm)
	num.occs <- sum(comm[, match(taxon, names(comm))], na.rm = TRUE)
	best.pred <- glmer_output$bestpred
	min.x <- min(comm[, match(best.pred, names(comm))], na.rm = TRUE)
	max.x <- max(comm[, match(best.pred, names(comm))], na.rm = TRUE)
	intercept <- fixef(glmer_output$bestmod)[2]
	slope <- fixef(glmer_output$bestmod)[1]
...
fit.models <- function(full_data, lidar, predictor, min.obs = 5){
	...
	for(i in 1:nrow(taxa_survey)){
		...
		if(sum(comm_data[, (length(predictor)+4)]) >= min.obs){		#Check there are enough presences to model
			...
			if(mixed){
				...
				coefs <- glmer.extract(glmer_output = model, taxa_data = full_data, comm = comm_data,
					taxon = rownames(taxa_survey)[i], coefs = coefs)
				}else{
					model <- fit.glm(comm = comm_data, predictor, taxon_ind = length(predictor) + 4)
					coefs <- glm.extract(glms_output = model, taxa_data = full_data, comm = comm_data,
						taxon = rownames(taxa_survey)[i], coefs = coefs)
					}
			models[[1]] <- model
